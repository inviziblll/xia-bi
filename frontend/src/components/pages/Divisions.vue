<template>
  <v-layout row>
    <v-flex xs12>
      <v-card-title>
        <v-subheader class="title">Дивизионы</v-subheader>
        <v-dialog v-model="dialog" persistent max-width="500px">
          <v-btn color="primary" dark slot="activator" class="mb-2">Создать дивизион</v-btn>
          <v-card>
            <v-card-title>
                ...
                <v-layout wrap>
                  ...
                  <v-flex xs12>
                    <v-select
                      ...
                      data-vv-as="Региональный менеджер"
                      required
                    ></v-select>
                  </v-flex>
                  <v-flex xs12>
...
<script>
  import {GET_DIVISIONS, GET_DIVISION, ADD_DIVISION, UPDATE_DIVISION, REMOVE_DIVISION, GET_USERS} from '@/vuex/types';
  ...
  export default {
    ...
    data () {
      return {
        ...
        headers: [
          {
            text: 'Название дивизиона',
            align: 'left',
            value: 'name'
          },
          {
            text: 'ФИО регионального менеджера',
            align: 'left',
            value: 'user.name'
          },
          {
    ...
    computed: {
      formTitle () {
        return this.editedIndex === -1 ? 'Создать дивизион' : 'Редактировать дивизион'
      },
      items(){
    ...
    methods:{
      editItem (itemId) {
        this.$store.dispatch(GET_DIVISION, itemId).then(
          result => {
      ...
      deleteItem (itemId) {
        confirm('Вы уверены что хотите удалить запись?') && this.$store.dispatch(REMOVE_DIVISION, itemId);
      },
        ...
        this.$validator.validateAll().then(result => {
          if (result) {
            if (this.editedIndex > -1) {
              this.$store.dispatch(UPDATE_DIVISION, this.editedItem);
            } else {
              this.$store.dispatch(ADD_DIVISION, this.editedItem)
            }
            this.close()
          }
        })
